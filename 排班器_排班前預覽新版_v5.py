...
def render() -> None:
    ...
    if not has_result:
        ...
        .preview-card {border:1px solid #d8dee6; border-radius:10px; overflow:hidden; margin-bottom:16px; background:white; box-shadow:0 1px 2px rgba(0,0,0,.04);}
        .preview-card-title {color:white; font-size:1.22rem; font-weight:700; padding:11px 14px;}
        .preview-table {width:100%; border-collapse:collapse; table-layout:fixed; font-size:1.08rem;}
        .preview-table th {color:white; font-weight:700; padding:8px 7px; border-right:1px solid rgba(255,255,255,.25);}
        .preview-table td {padding:8px 7px; text-align:center; border-top:1px solid #e5e9ef;}
        .preview-table tbody tr:nth-child(even) {background:#f8fafc;}
        .preview-table .preview-name {text-align:left; padding-left:14px; font-weight:600;}
        .preview-table .preview-total {font-weight:800;}
        .preview-table .preview-empty {color:#7b8794; text-align:center; padding:16px;}
        </style>
        """, unsafe_allow_html=True)
        ...
        with left_col:
            st.markdown(preview_table_html("急專醫師", er_rows, "#4F81BD"), unsafe_allow_html=True)
            st.markdown(preview_table_html("外援醫師", external_rows, "#E26B0A", "本月無外援醫師"), unsafe_allow_html=True)
        with right_col:
            st.markdown(preview_table_html("支援醫師", support_rows, "#70AD47"), unsafe_allow_html=True)
