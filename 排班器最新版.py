...
def solve(payload: dict) -> dict:
    ...
    # 全月 24 小時班數量：只作為軟目標，不強迫一定達成，避免因此造成無解。
    ...
    if desired_doubles is not None and desired_weight > 0:
        total_doubles = model.NewIntVar(0, len(days), "total_doubles")
        model.Add(total_doubles == sum(doubles.values()))
        difference = model.NewIntVar(0, len(days), "double_target_difference")
        model.AddAbsEquality(difference, total_doubles - int(desired_doubles))
        objective.append(desired_weight * difference)

    # 個別醫師 24 小時班設定（選填）。
    # auto：完全交由求解器；forbidden：硬限制為 0；max：硬性上限；prefer：軟性目標。
    # max／prefer 的數量由 UI 另外指定，因此不再把「1 個、2 個」寫死在模式名稱裡。
    for p in names:
        doctor = by_name[p]
        mode = doctor.get("double_shift_mode", "auto")
        requested = max(0, int(doctor.get("double_shift_count", 0)))
        double_count = model.NewIntVar(0, len(days), f"double_count_{p}")
        model.Add(double_count == sum(doubles[p, i] for i in range(len(days))))

        if mode == "forbidden":
            model.Add(double_count == 0)
        elif mode == "max":
            model.Add(double_count <= requested)
        elif mode == "prefer":
            diff = model.NewIntVar(0, len(days), f"double_pref_diff_{p}")
            model.AddAbsEquality(diff, double_count - requested)
            objective.append(int(payload.get("individual_double_preference_weight", 30)) * diff)
...
def preview_rows(payload: dict) -> list[dict]:
    """建立排班前預覽資料，正確拆分急專、外援與支援醫師的平／假日班數。"""
    holidays = set(payload["holidays"])
    doctors = payload["doctors"]
    by_name = {d["name"]: d for d in doctors}
    fixed = payload["fixed_shifts"]

    # 外援都是指定班，直接依指定日期計算平日／假日。
    external_holiday_counts = {}
    for item in fixed:
        doctor = by_name.get(item["doctor"])
        if doctor and doctor["type"] == "external" and item["date"] in holidays:
            external_holiday_counts[item["doctor"]] = external_holiday_counts.get(item["doctor"], 0) + 1

    # 急專假日班數需與 solve() 的硬限制完全一致，才能讓預覽等於實際求解目標。
    ers = [d["name"] for d in doctors if d["type"] == "er"]
    support_holidays = sum(int(d.get("holiday_quota", 0)) for d in doctors if d["type"] == "support")
    external_holidays = sum(external_holiday_counts.values())
    er_holiday_targets = {}
    if ers:
        er_slots = len(holidays) * 2 - support_holidays - external_holidays
        low, remainder = divmod(max(0, er_slots), len(ers))
        for pos, name in enumerate(ers):
            er_holiday_targets[name] = low if pos < len(ers) - remainder else low + 1

    rows = []
    for doctor in doctors:
        total = int(doctor["target_shifts"])
        if doctor["type"] == "er":
            holiday = er_holiday_targets.get(doctor["name"], 0)
        elif doctor["type"] == "external":
            holiday = external_holiday_counts.get(doctor["name"], 0)
        else:
            holiday = int(doctor.get("holiday_quota", 0))
        rows.append({
            "姓名": doctor["name"],
            "類別": {"er": "急專", "external": "外援", "support": "支援"}[doctor["type"]],
            "平日": total - holiday,
            "假日": holiday,
            "總班數": total,
        })
...
def preview_table_html(title: str, rows: list[dict], header_color: str, empty_text: str = "本月無資料") -> str:
    """建立排班前預覽用的彩色表格。"""
    body = []
    if rows:
        for row in rows:
            body.append(
                "<tr>"
                f"<td class='preview-name'>{escape(str(row['姓名']))}</td>"
                f"<td>{int(row['平日'])}</td>"
                f"<td>{int(row['假日'])}</td>"
                f"<td class='preview-total'>{int(row['總班數'])}</td>"
                "</tr>"
            )
    else:
        body.append(f"<tr><td colspan='4' class='preview-empty'>{escape(empty_text)}</td></tr>")
    return f"""
    <div class='preview-card'>
      <div class='preview-card-title' style='background:{header_color};'>{escape(title)}</div>
      <table class='preview-table'>
        <thead>
          <tr style='background:{header_color};'>
            <th>姓名</th><th>平日</th><th>假日</th><th>總班數</th>
          </tr>
        </thead>
...
def legacy_preview_table_html(rows: list[dict], variant: str) -> str:
    """保留原版排班器的排班前預覽表格樣式。"""
    headers = ("姓名", "平日", "假日", "總班數")
    html = [f"<table class='preview-table preview-table-{variant}'><thead><tr>"]
    html.extend(f"<th>{header}</th>" for header in headers)
    html.append("</tr></thead><tbody>")
    for row in rows:
        html.append("<tr>" + "".join(f"<td>{escape(str(row[header]))}</td>" for header in headers) + "</tr>")
    return "".join(html) + "</tbody></table>"
...
def render() -> None:
    ...
    if not has_result:
        ...
        st.markdown("""
        <style>
        .preview-heading {font-size: 2.0rem; font-weight: 700; margin: 1.25rem 0 0.25rem; color: #4f81bd; border-bottom: 2px solid #4f81bd; padding-bottom: 0.35rem;}
        .preview-section-er, .preview-section-external, .preview-section-support {font-size: 1.45rem; font-weight: 700; margin: 1rem 0 0.45rem;}
        .preview-section-er::before, .preview-section-external::before, .preview-section-support::before {content: ""; display: inline-block; width: 0.82rem; height: 0.82rem; margin-right: 0.65rem; border-radius: 50%; vertical-align: 0.04rem;}
        .preview-section-er {color: #477a44;}
        .preview-section-er::before {background: linear-gradient(135deg, #80d77b, #3d9860);}
        .preview-section-external {color: #e26b0a;}
        .preview-section-external::before {background: linear-gradient(135deg, #ffc06e, #e26b0a);}
        .preview-section-support {color: #4f81bd;}
        .preview-section-support::before {background: linear-gradient(135deg, #8db8ed, #3f70ad);}
        div[data-testid="stMetricLabel"] {font-size: 1.15rem;}
        div[data-testid="stMetricValue"] {font-size: 2.35rem;}
        .preview-table {width: 100%; border-collapse: collapse; font-size: 1.15rem; margin: 0 0 0.8rem;}
        .preview-table th {color: white; border: 1px solid rgba(0, 0, 0, 0.16); padding: 0.6rem 0.75rem; text-align: center;}
        .preview-table-er th {background: #4e9a64;}
        .preview-table-external th {background: #eb7000;}
        .preview-table-support th {background: #4f81bd;}
        .preview-table td {border: 1px solid #d0d0d0; padding: 0.58rem 0.75rem; text-align: center; background: #ffffff; color: #333;}
        .preview-table td:first-child {font-weight: 700; text-align: left;}
        </style>
        """, unsafe_allow_html=True)
        ...
        b.metric("急專目標班數", sum(r["總班數"] for r in rows if r["類別"] == "急專"))
        c.metric("外援鎖定班數", sum(r["總班數"] for r in rows if r["類別"] == "外援"))
        d.metric("支援分配班數", sum(r["總班數"] for r in rows if r["類別"] == "支援"))

        er_rows = [r for r in rows if r["類別"] == "急專"]
        external_rows = [r for r in rows if r["類別"] == "外援"]
        support_rows = [r for r in rows if r["類別"] == "支援"]
        left_col, right_col = st.columns([1, 1], gap="large")
        with left_col:
            if er_rows:
                st.markdown("<h3 class='preview-section-er'>急專醫師</h3>", unsafe_allow_html=True)
                st.markdown(legacy_preview_table_html(er_rows, "er"), unsafe_allow_html=True)
            if external_rows:
                st.markdown("<h3 class='preview-section-external'>外援醫師（鎖定班）</h3>", unsafe_allow_html=True)
                st.markdown(legacy_preview_table_html(external_rows, "external"), unsafe_allow_html=True)
        with right_col:
            if support_rows:
                st.markdown("<h3 class='preview-section-support'>支援醫師（分配班）</h3>", unsafe_allow_html=True)
                st.markdown(legacy_preview_table_html(support_rows, "support"), unsafe_allow_html=True)
        st.markdown("### 24 小時班設定")
        # 實務上 0～6 個已足夠涵蓋一般排班需求；避免顯示到 30、31 個這種沒有實際意義的上限。
        max_monthly_doubles = 6
        saved_double_target = min(int(st.session_state.get("schedule_double_target", 2)), max_monthly_doubles)
        double_target = st.slider(
            "全月 24 小時班目標",
            min_value=0,
            max_value=max_monthly_doubles,
            value=saved_double_target,
            step=1,
            help="這是全月總量的軟目標；若與硬性規則衝突，求解器可以少排或多排，以避免整張班表無解。",
        )
        payload["desired_double_shifts"] = double_target
        payload["desired_double_shifts_weight"] = 80
        payload["individual_double_preference_weight"] = 30
        st.caption(f"目前設定：全月約 {double_target} 個 24 小時班；實際數量仍以硬性規則與整體班表品質為優先。")
        ...
        option_labels = list(individual_options)
        with st.expander("個別醫師 24 小時班設定（選填）", expanded=False):
            st.caption("預設全部為「自動」。特殊需求時才設定；選「最多」或「希望」後可自行輸入數量，不會因連假 7 天以上自動放寬。")
            er_doctors = [doctor for doctor in payload["doctors"] if doctor["type"] == "er"]

            # 固定欄位標題，避免 selectbox 可見標籤造成不同列的元件上下偏移。
            header = st.columns([1.25, 2.0, 2.0, 1.35])
            header[0].markdown("**醫師**")
            header[1].markdown("**設定**")
            header[2].markdown("**數量**")
            header[3].markdown("**說明**")

            for doctor in er_doctors:
                row = st.columns([1.25, 2.0, 2.0, 1.35], vertical_alignment="center")
                row[0].markdown(f"**{escape(doctor['name'])}**")

                mode_key = f"double-mode-{doctor['name']}"
                label = row[1].selectbox(
                    "設定",
                    option_labels,
                    index=0,
                    key=mode_key,
                ...
                mode = individual_options[label]
                doctor["double_shift_mode"] = mode

                # 一個 24 小時班占 2 個班次；個人可輸入的上限依其目標班數計算。
                max_personal_doubles = max(1, int(doctor["target_shifts"]) // 2)
                if mode in {"max", "prefer"}:
                    count_key = f"double-count-{doctor['name']}"
                    default_count = min(int(st.session_state.get(count_key, 1)), max_personal_doubles)
                    count = row[2].number_input(
                        "數量",
                        min_value=1,
                        max_value=max_personal_doubles,
                        value=default_count,
                        step=1,
                        key=count_key,
                        label_visibility="collapsed",
                    )
                    row[3].caption(f"{label} {int(count)} 個")
                    doctor["double_shift_count"] = int(count)
                else:
                    # 用相同欄位保留空間，使每一列視覺上保持整齊。
                    row[2].caption("—")
                    row[3].caption("由求解器決定" if mode == "auto" else "固定 0 個")
                    doctor["double_shift_count"] = 0
        if st.button("產生 CP-SAT 班表", type="primary", use_container_width=True):
            ...
            st.session_state["schedule_double_target"] = double_target
